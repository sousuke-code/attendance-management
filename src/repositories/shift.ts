import { eq, and, inArray,aliasedTable, desc, asc } from "drizzle-orm";
...
export async function getShiftSwapLists(){
    return db.select().from(shiftSwapDetails).where(eq(shiftSwapDetails.status, "pending")).orderBy(asc(shiftSwapDetails.shiftDate));
}
